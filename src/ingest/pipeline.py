...
def delete_source_files(notebook_id: int, source_id: int) -> None:
    dest_dir = SOURCES_DIR / str(notebook_id) / str(source_id)
    if dest_dir.exists():
        try:
            shutil.rmtree(dest_dir)
        except OSError as e:
            logger.warning(f"Failed to remove source files at {dest_dir}: {e}")


def delete_notebook_source_files(notebook_id: int) -> None:
    dest_dir = SOURCES_DIR / str(notebook_id)
    if dest_dir.exists():
        try:
            shutil.rmtree(dest_dir)
        except OSError as e:
            logger.warning(f"Failed to remove notebook source dir {dest_dir}: {e}")
...
def _chunk_to_text_row(chunk, notebook_id: int, source_id: int) -> Dict[str, Any]:
    citation = {
    ...
    if chunk.metadata:
        citation.update({k: v for k, v in chunk.metadata.items() if k not in citation})
    return {
...
def ingest_chunks(
    ...
    if replace_existing:
        ...
        if existing:
            memory.delete_chunk_texts_by_source(existing["id"])
